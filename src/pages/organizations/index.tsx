        ...
        >
          ...
          >
            ...
            >
              ...
              >
                <Link href={`/organizations/${organization?.id}/settings`}>{organization?.name || 'Organization'}</Link>
              </Heading>
              <Text color={'gray.500'}>
                by <Link href={`/@${organization?.owner?.username}`}>{organization?.owner?.displayName}</Link>
              </Text>
              <Tooltip
                label={toActualTime(organization?.updatedAt as string)}
                cursor={'help'}
              >
                <Flex
                  align={'center'}
                ...
                >
                  <Icon as={BiRefresh} />
                  <Text color={'gray.500'}>{toRelativeTime(organization?.updatedAt as string)}</Text>
                </Flex>
              </Tooltip>
            </Flex>
          </Skeleton>
      ...
      <Tr key={key}>
        <Td>
          ...
          >
            ...
            >
              <Skeleton isLoaded={!skeleton}>
                <Text fontWeight={'bold'}>{!skeleton ? organization?.name : 'Organization Name'}</Text>
                <Text
                  size={'sm'}
                  variant={'subtext'}
                  textUnderlineOffset={4}
                >
                  Owned by{' '}
                  {!skeleton ? (
                    <Link href={`/@${organization?.owner?.username}`}>{organization?.owner?.displayName}</Link>
                  ) : (
                    'Organization Owner'
                  )}
                </Text>
                <Tooltip label={toActualTime(organization?.updatedAt as string)}>
                  ...
                  >
                    ...
                    >
                      ...
                      {!skeleton ? (
                        <Link href={`/@${organization?.updatedBy?.username}`}>
                ...
                <Text
                  size={'sm'}
                  variant={'subtext'}
                  maxW={{ base: '500px', md: '384px', lg: '500px' }}
                  overflow={'hidden'}
...
export default function PlatformOrganizations() {
  ...
  const joinOrganizationPrompt = useCallback(
    ...
    (query: string) => {
      ...
      const filtered = organizations.filter((organization: Organization) => {
        return (
          ...
          organization.owner?.displayName?.toLowerCase().includes(query.toLowerCase())
        );
      });
          ...
          >
            ...
            <ButtonGroup isAttached>
              ...
              >
                <IconButton
                  aria-label={'List'}
                  variant={view === 'list' ? 'solid' : 'unselected'}
                  onClick={() => {
                    setView('list');
                  }}
                  icon={<BsListUl />}
              ...
              >
                <IconButton
                  aria-label={'Grid'}
                  variant={view === 'grid' ? 'solid' : 'unselected'}
                  onClick={() => {
                    setView('grid');
                  }}
                  icon={<BiGrid />}
        ...
        <Box w={'full'}>
          ...
          >
            {view === 'list' ? (
              ...
              >
                {organizationsLoading
                  ? Array.from({ length: 6 }).map((_, i) => <GridEntry key={i} />)
                  : filteredOrganizations.map((organization: Organization) => (
                      <GridEntry
                        key={organization.id}
                        organization={organization}
