    ...
    <Tr key={key}>
          ...
          <Flex flexDir={'column'} justify={'center'}>
            <Skeleton isLoaded={!skeleton}>
              <Text fontWeight={'bold'}>
                {!skeleton ? invitation?.organization?.name : "N/A"}
              </Text>
            </Skeleton>
            <Skeleton isLoaded={!skeleton}>
              <Text
                variant={'subtext'}
              >
                Owned by {invitation?.createdBy?.displayName}
              </Text>
            </Skeleton>
          </Flex>
        </Stack>
      ...
      <Td isNumeric>
        <Skeleton isLoaded={!skeleton}>
          <ButtonGroup>
            <Button
              onClick={() => { action(invitation, 'ignore') }}
              size={"sm"}
              variant={"solid"}
              colorScheme='red'
...
export default function UserInvitationsModal({
  ...
  const actOnInvitation = useCallback(async (invitation: OrganizationInvitation, action: 'accept' | 'ignore') => {
    setInvitationsLoading(true);
    user.getIdToken().then(async (token: string) => {
